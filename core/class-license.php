...
class License {
	...
	public function activate_license() {
		if ( ! isset( $_POST['nonce'] ) || empty( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'stobokit_license' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Sorry, not verified.', 'review-follow-up-for-wooCommerce' ),
				)
			);
		}
				...
				switch ( $data->error ) {
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'review-follow-up-for-wooCommerce' ),
							date_i18n( get_option( 'date_format' ), strtotime( $data->expires, current_time( 'timestamp' ) ) )
						);
						break;
					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'review-follow-up-for-wooCommerce' );
						break;
					case 'missing':
						$message = esc_html__( 'Invalid license.', 'review-follow-up-for-wooCommerce' );
						break;
					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'review-follow-up-for-wooCommerce' );
						break;
					case 'invalid_item_id':
						/* translators: 1: Plugin name */
						$message = esc_html__( 'This appears to be an invalid license key for this product.', 'review-follow-up-for-wooCommerce' );
						break;
					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'review-follow-up-for-wooCommerce' );
						break;
					default:
						$message = esc_html__( 'An error occurred, please try again.', 'review-follow-up-for-wooCommerce' );
						break;
				}
			}
		}
		...
		if ( ! empty( $message ) ) {
			...
			wp_send_json_success(
				array(
					'message'     => esc_html__( 'License activated succesfully!', 'review-follow-up-for-wooCommerce' ),
					'status'      => 'active',
	...
	public function deactivate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( isset( $license_data->license ) && in_array( $license_data->license, array( 'deactivated', 'failed' ), true ) ) {
				...
				wp_send_json_success(
					array(
						'message' => esc_html__( 'License deactivated succesfully!', 'review-follow-up-for-wooCommerce' ),
						'status'  => 'inactive',
					)
				);
			}
