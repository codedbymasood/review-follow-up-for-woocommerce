...
class List_Table extends \WP_List_Table {
	...
	public function get_columns() {
		return apply_filters(
			...
			array(
				...
				'id' => esc_html__( 'ID', 'review-follow-up-for-wooCommerce' ),
			)
		);
	}
	...
	protected function extra_tablenav( $which ) {
		if ( 'top' === $which && apply_filters( $this->id . '_table_allow_export_csv', false ) ) {
			...
			echo '<a href="' . esc_url( $export_url ) . '" class="button button-secondary">' . esc_html__( 'Export All to CSV', 'review-follow-up-for-wooCommerce' ) . '</a>';
			echo '</div>';
		}
	...
	public function handle_csv_export() {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to export data.', 'review-follow-up-for-wooCommerce' ) );
		}

		// Verify nonce.
		if ( isset( $_GET['_wpnonce'] ) && ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), $this->id . '_export_csv_nonce' ) ) {
			wp_die( esc_html__( 'Invalid nonce.', 'review-follow-up-for-wooCommerce' ) );
		}
