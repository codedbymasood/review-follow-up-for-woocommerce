	...
	private function render_repeater_field( $field, $post, $field_value ) {
		...
		if ( ! empty( $repeater_values ) && is_array( $repeater_values ) ) {
			foreach ( $repeater_values as $index => $repeater_value ) {
				...
				echo '<span class="repeater-title">' . esc_html__( 'Item: ', 'review-follow-up-for-wooCommerce' ) . ' ' . esc_html( ( (int) $index + 1 ) ) . '</span>';
		...
		echo '<div class="repeater-header">';
		echo '<span class="repeater-title">' . esc_html__( 'Item: ', 'review-follow-up-for-wooCommerce' ) . ' {INDEX_DISPLAY}</span>';
		echo '<span class="remove-item"><span class="dashicons dashicons-no-alt"></span></span>';
		echo '</div>';
		echo '<div class="repeater-content">';
		...
		echo '</div>';
		echo '<button type="button" class="button add-repeater-item">' . esc_html__( 'Add Item', 'review-follow-up-for-wooCommerce' ) . '</button>';
	}
		...
		switch ( $field['type'] ) {
			...
			case 'media':
				...
				printf(
					...
					esc_html__( 'Select Media', 'review-follow-up-for-wooCommerce' )
				);

				echo '<button type="button" class="button remove-media-button">' . esc_html__( 'Remove All', 'review-follow-up-for-wooCommerce' ) . '</button>';
