 ...
 * Hooks class.
 *
 * @package review-follow-up-for-wooCommerce\common\includes\
...
class Hooks {
		...
		revifoup()->emailer->register_shortcode(
			...
			function ( $args ) {
				...
				foreach ( $items as $item_id => $item ) {
					...
					$html .= '<a href="' . esc_url( $product_url . '#tab-reviews' ) . '">' . esc_html__( 'Leave a Review', 'review-follow-up-for-wooCommerce' ) . '</a>';
					$html .= '</div>';
				}
	...
	public function send_reward_email( $row = array() ) {
		...
		$subject     = get_option( 'revifoup_review_reward_email_subject', esc_html__( 'Your {discount} discount code is here! 🎉', 'review-follow-up-for-wooCommerce' ) );
		$heading     = get_option( 'revifoup_review_reward_email_heading', esc_html__( 'Thank You for Your Review! Here\'s Your {discount} Discount Code', 'review-follow-up-for-wooCommerce' ) );
